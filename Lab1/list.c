...
{
	struct list_node *allo;
	allo=(struct list_node*)malloc(sizeof(struct list_node));
	allo->key=key;
	allo->next= NULL;
	allo->prev=NULL;
...
{
	struct list_node *now_node;
	now_node=head->next;
	while (!(now_node->key== search_key)&&!(now_node->key==-1)){
...
int insert_sorted_by_key (struct list_node *head, struct list_node *new_node)
{
	struct list_node *now_node;
	now_node=head->next;
	while ((now_node->key < new_node->key)&&(now_node->key != -1)){
		now_node=now_node->next;
	}
